...
```mermaid
graph TB
    A[Humanoid Robot\n30-50 DOF] --> B[Challenges]

    B --> C[Whole-Body Control\nMaintain balance]
    B --> D[High Dimensionality\n50+ action dimensions]
    B --> E[Dexterous Hands\nComplex grasping]
    B --> F[Task Diversity\n1000s of tasks]
    ...
    G --> H[Language Conditioning\nNatural task specification]
    G --> I[Learned Priors\nFrom demonstrations]
    G --> J[Multimodal Perception\nVision + proprioception]
...
```mermaid
graph TB
    A[Vision Input\nHead + Wrist Cameras] --> B[Vision Encoder\nCLIP-ViT]
    C[Language Input\nTask instruction] --> D[Language Encoder\nT5]
    E[Proprioception\nJoint angles, IMU] --> F[State Encoder\nMLP]
...
graph LR
    ...
    B -->|Stable| C[Execute Action\nNormal]
    B -->|Unstable| D[Reject Action\n+ Replan]

    C --> E[Update Robot State]
    D --> F[Balance Recovery\nAdjust torso/legs]
...
```mermaid
graph TB
    A[Dexterous Hand\n12-20 DOF] --> B[Challenges]

    B --> C[High Dimensionality\nComplex action space]
    B --> D[Contact Dynamics\nFinger-object interaction]
    B --> E[Tactile Feedback\nForce sensing]
    B --> F[Precision Grasping\nSub-millimeter accuracy]
    ...
    G --> H[Learned Manipulation\nFrom demonstrations]
    G --> I[Tactile Encoders\nProcess touch sensors]
    G --> J[Grasp Prediction\nPre-shape + refine]
...
```mermaid
graph TB
    A[Vision Input\nHead camera] --> B[Shared Encoder\nScene understanding]

    B --> C[Left Arm Policy\n7 DOF]
    B --> D[Right Arm Policy\n7 DOF]
    B --> E[Coordination Module\nSync timing]
...
```mermaid
graph LR
    A[Web Videos\n100M+ videos] --> B[Pre-training\nVideo prediction]

    B --> C[Pre-trained VLM\nUnderstands actions]

    C --> D[Robot Fine-tuning\n10k demos]
