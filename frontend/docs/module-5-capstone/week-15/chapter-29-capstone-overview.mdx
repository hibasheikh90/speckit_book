...
```mermaid
graph TB
    A[Human Operator] -->|"Bring me the\nblue box from shelf A"| B[VLA Command Parser]

    B --> C[Task Planner\nBreak into subtasks]

    C --> D[Navigation System\nPath to shelf A]
    C --> E[Manipulation System\nGrasp blue box]
    C --> F[Return Navigation\nPath to operator]

    D --> G[Perception Pipeline\nVision + sensors]
    E --> G
    F --> G

    G --> H[Whole-Body Controller\n30+ DOF coordination]
...
graph TB
    ...
    subgraph "Control Layer"
        ...
        K[Arm Controllers\n2x 7-DOF]
        L[Hand Controllers\n2x gripper/dexterous]
        M[Locomotion Controller\nBipedal walking]
    end
